...
<ul class="nav nav-tabs" id="myTab" role="tablist">
  ...
  <li class="nav-item" role="presentation" title="찜">
    <button class="nav-link" id="contact-tab" data-bs-toggle="tab" data-bs-target="#contact-tab-pane" type="button" role="tab" aria-controls="contact-tab-pane" aria-selected="false">찜한 오디션</button>
  </li>
</ul>
		...
		if(type == "진행" || type == "마감" || type == "전체"){
						...
						for(let i=0; i<list.length; i++){
							tbodyEle.innerHTML +=
								'<tr>' +
									...
									'<td>'+list[i].audition_situation+'</td>'+
								'</tr>';
						}
					}
				},
			...
			$.ajax({
				...
				success : function(list){
					...
					if(list.length == 0){
						tbodyEle.innerHTML += '----찜한 오디션이 없습니다----';
					}else{
						for(let i=0; i<list.length; i++){
							tbodyEle.innerHTML +=
								'<tr>' +
