...
<body>
	...
	<c:choose>
		<c:when test="${paidService == null }">
			<div class="text-center"
				style="position: absolute; width: 100%; height: 100%; background-color: white; background-image: url(/resources/img/blur.jpg); background-repeat: no-repeat; background-position: center;">
				<div style="margin: 20%">
					<h1 class="fw-bolder">통계 자료 열람을 위해 결제가 필요합니다.</h1>
					<button onclick="location.href='/PaidServiceboard/list'">유료
						서비스 안내 페이지로 이동</button>
				</div>
			</div>
		</c:when>
		<c:otherwise>
			...
			<div class="container " style="display: flex;">
				...
				<c:choose>
					...
					<c:otherwise>
							...
							<div style="background-color: white; width: 100%;"class="pt-5" >
...
<c:choose>
	<c:when test="${dto.member_gender eq '여성' }">
		<c:choose>
...
<c:choose>
	<c:when test="${dto.member_gender eq '남성' }">
		<c:choose>
